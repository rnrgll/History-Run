...
public class POP_Switch_Slide : MonoBehaviour
{
    ...
    public static bool GetVolume(AudioMixer mixer, string exposedName)
    {
        mixer.GetFloat(exposedName, out float volume);
        if (volume <= -80f) // 일반적으로 -80dB 미만은 뮤트 상태로 간주합니다.
        {
            Debug.Log("is muted");
            return true;
        }
    ...
    {         
        ...
        else  
        {
            ...
            StartCoroutine(SwitchCoroutineOff());
            btn1.interactable = false;
            audioMixer.SetFloat(exposedName, Mute);
            isoff = true;
        }
    }
